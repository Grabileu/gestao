...
const navigation = [
  ...
  { 
    ...
    submenu: [
      ...
      { name: "Configurações RH", href: "HRConfig", icon: Settings },
    ]
  },
  { 
    name: "CEASA", 
...
export default function Layout({ children, currentPageName }) {
  ...
  const [expandedMenus, setExpandedMenus] = useState(["RH", "CEASA", "Quebras de Caixa", "Relatórios"]);

  // Normalize path helper
  const normalizePath = (p) => {
    if (!p) return '/'
    // if App passes 'Dashboard' string, treat as '/'
    if (p === 'Dashboard') return '/'
    return p.replace(/\/+$/g, '') || '/'
  }

  // Auto-expand parent menus when current page matches a submenu
  useEffect(() => {
    const current = normalizePath(currentPageName)
    const parentsToOpen = []
    navigation.forEach((item) => {
      if (item.submenu) {
        const matched = item.submenu.some(sub => normalizePath(createPageUrl(sub.href)) === current || current.startsWith(normalizePath(createPageUrl(sub.href)) + '/'))
        if (matched) parentsToOpen.push(item.name)
      }
    })
    // Auto-expand matched parent menus
    if (parentsToOpen.length) {
      setExpandedMenus(prev => Array.from(new Set([...prev, ...parentsToOpen])))
    }
  }, [currentPageName])
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
          ...
          <nav className="flex-1 px-3 py-6 space-y-1 overflow-y-auto">
            {navigation.map((item) => {
              if (item.submenu) {
                ...
                const hasActiveChild = item.submenu.some(sub => createPageUrl(sub.href) === currentPageName);
                return (
                  <div key={item.name}>
                    <button
                    ...
                    {isExpanded && (
                      <div className="ml-4 mt-1 space-y-1">
                        {item.submenu.map((subItem) => {
                          const isActive = createPageUrl(subItem.href) === currentPageName;
                          return (
                            <Link
                              key={subItem.name}
              ...
              const isActive = createPageUrl(item.href) === currentPageName;
              return (
                <Link
                  key={item.name}
      ...
      <div className="lg:pl-64">
        ...
        <header className="sticky top-0 z-30 flex items-center h-16 px-4 bg-slate-900/80 backdrop-blur-xl border-b border-slate-700/50 lg:hidden">
          ...
          <span className="ml-4 text-lg font-semibold text-white">HR System</span>
        </header>
